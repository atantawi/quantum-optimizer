...
- **Redoing the contraction argument** for the new map (analysed for the current one in `docs/convergence-tuned-r-star`, a local working directory that is not part of this repository). Measurements show no practical change: iterations 5–17 against 6–18, zero failures across 13 budgets from 1.0001× to 1e4× the floor.
